...
We would first show the basic knowledge of it, then
provide its background theory and you can have a look
...
function is the risk of dying at time $t$, you can think 
it as time-specific risk of having event of die.

As a reminder, we noted that the Cox PH model 
...
$\exp({\beta})$ is called hazard ratios (HR). 
If $\beta > 0$ (or $\exp({\beta})$ > 1), this implies as 
...
Now, slopes $\beta$ is interpretable as difference in log(hazard) 
with difference in $X_{1}$ at a given time(t). 

### Background Theory

To get a sense of what the hazard function is, let's first 
