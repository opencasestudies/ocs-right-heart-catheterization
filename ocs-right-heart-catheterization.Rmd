...
can be a good choice to estimate the survival rate. In 
general, the survival probability is calculated as one 
minus the probablity of death. At each time interval, the 
probability of death equals the number of patients who
died divided by the number of patients at risk of dying. 
...
The Kaplan-Meier estimator is a non-parametric method 
used to estimate the probability of survival. You can 
think of the method as a series of declining horizontal 
steps which, ith a large enough sample size, approaches
the true survival function for that population.

The `Surv()` function in the 
[`survival` package](https://www.rdocumentation.org/packages/survival/versions/2.11-4)
uses a specialize class from the `survival` package 
...
To understand the hazard function, let's consider the 
...
$$S(t) = Pr(T \geq t) = \int_{t}^{\infty} f(t) dt$$
...
OK, now we are ready to get back to the Cox PH
...
value differs between the $i^{th}$ and
...
Cox PH model (we will do this below). 


### Model Application

We can fit a Cox PH model using the `coxph()` function, 
...
Then, we can fit the Cox PH model using the `coxph()` 
...
object to note right censored observations. We can 
tell R which variables to exclude using the `-` syntax. 
...
argument. 
```{r}
res_exp <- tidy(fit1, exponentiate = TRUE, 
