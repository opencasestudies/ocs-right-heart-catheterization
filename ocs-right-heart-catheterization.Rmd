...
survival (or mortality) due to treatment with a RHC, using an observational dataset. To answer this question, we will introduce tools used in survival analysis, including propensity scores and Cox Proportional Hazards models. We will eventually be able to make and interpret figures like this one, which illustrates the trajectory of survival probabilities over time for people who did or did not receive RHC.
...
influence the outcome, such as `blood pressure`, or  `age`. In our case, we are interested in the effect 
of `RHC`, but we also know there are additional 
covariates that might effect the outcome. So, we 
...
We will first give an overview of the concepts behind a Cox PH model, then
provide some background theory that you can take a look at
if you are interested.

### Overview of the Cox PH model

The Cox PH model is expressed using the hazard function
$h(t)$ at time $t$. The intepretation of the hazard 
function is the risk of dying at time $t$, so you can think 
it as a time-specific chance of dying. For the $i^{th}$ observation and a given
set of covariates 
$\mathbf{X}_i = \{ X_{i1}, \ldots X_{ip} \}$, 
the Cox PH model has the form 

$$h(t | \mathbf{X}_i)= h_0(t)  e^{\beta_1 X_{i1} + \ldots \beta_p X_{ip}} 
= h_0(t) e^{\beta \mathbf{X}_{i}} $$

where the coefficients $\beta = \{\beta_1, \ldots, \beta_p \}$
are the effect sizes of the covariates $\mathbf{X}_i$. The Cox PH model allows us to investigate the relationship between the covariates $\mathbf{X}_i$ and the chance of death, considering that the chance of death may vary over time accoring to $h_0(t)$, which is called the "baseline hazard function at time $t$". 

**Note**: If you take the natural log of the hazard function, you get
a linear model with a baseline hazard function (intercept)
that varies over time. 

Here, $h_0(t)$ is the baseline hazard at time $t$. 
This is the hazard if all the $\mathbf{X}_i$ are 0. A 
**key assumption** of the Cox PH model is 
that the hazard curves for groups of observations 
should be proportional and cannot cross.

To be specific, consider the $i^{th}$ and
$j^{th}$ observations and suppose we have only one covariate $X_1$, with 
observed values $x$ and $x^{'}$ for the $i^{th}$ and
...
phrasing this is that the hazard for any observation
is a constant multiple of the hazard for any other observation. 

How do we interpret the coefficients from the Cox PH model? The values $e^{\beta}$ are called hazard ratios (HRs). 
If $\beta > 0$ (or $e^{\beta}$ > 1), this implies that as 
$X_{1}$ increases, the hazard increases and thus the 
...
If you take log of the above formula, you will get

$$ log(h_i(t|X_{1}  = x)) - log (h_j(t|X_{1} = x^{'})) = \beta (x-x^{'})$$

Now, we can see that the slopes $\beta$ are interpretable as differences in the log(hazard) 
for a difference in $X_{1}$ at a given time, $t$. 

Another transformation of the hazard function, $h(t)$ that can be useful is the  "complementary log-log" model (or $log(-log(1-h(t)))$). This transformation can be useful when the probability of an event is very small or very large. 



### Background theory

To get a sense of what the hazard function is in a more detailed mathematical way, let's first 
consider the survival function. If $T$ is a continous random 
variable representing the time of death, with probability 
distribution $f(t)$ and cumulative distribution $F(t)$ that 
represents the survival time, we can describe the cumulative 
...
the data after matching based on propensity scores. However, to isolate the effect of interest, we still want to include the covariates in the model to estimate a conditional treatment effect.
...
model output as a `data.frame` object. 
...
What does this model output tell us? The coefficient here (`estimate`) is the 
$\beta$ coefficient from the model formulation described above, where we have only one covariate, RHC. More specifically, we have fit the model:
$$h(t | X)= h_0(t)  e^{\beta_1 X} $$

where $X=1$ if the patient received RHC and $X=0$ if not and $h_0(t)$ is the baseline hazard at time $t$. Again referring to the results from above, we have:

$$\frac{h(t| RHC)}{h(t|no RHC)}=\frac{h_0(t) e^{\beta * 1}}{h_0(t) e^{\beta * 0}}=e^{\beta}$$


Our model output tells us that we on average have multiplicative change in hazard of $e^{\beta}$=exp(`r round(res$estimate, 3)`)=`r round(exp(res$estimate),3)`, comparing a person with RHC ($X=1$) to one without ($X=0$). This exponentiated coefficient, whose value is `r round(exp(res$estimate),3)`, is the hazard ratio comparing someone with RHC to someone without RHC. If the exponentiated 
coefficient is larger than 1, the model estimates that the variable will increase 
the hazard of the event occuring (or decrease the 
survival probability). The model output also contains a p-value and 95% confidence interval for the coefficient; in this case, we can see that $\beta$ is statistically significantly different from 0 since the p-value is < 0.05 and the confidence interval does not contain the null value of 0.
...
```{r}
res_exp <- tidy(fit_uni1, exponentiate = TRUE, 
            conf.int = TRUE, conf.level = 0.95)
res_exp
```

Note that now, we want to check whether the confidence interval contains 1, which is the null value for the hazard ratio. We can see that the significance results match those from the non-exponentiated model output, which makes sense.
...
res <- tidy(fit_uni2, exponentiate=TRUE,
            conf.int = TRUE, conf.level = 0.95)
res
```

We can see that for each additional year in age, the hazard is estimated to change by a multiplicative factor of `r round(res$estimate,3)` and that this estimate is again statistically significantly different from the null value of 1.
...
res <- tidy(fit_uni3, exponentiate=TRUE,
            conf.int = TRUE, conf.level = 0.95)
res
```
...
function and include all the covariates in our dataset 
...
Again, we can report the results on the exponential scale, so that the esimates are interpretable as hazard ratios:
...
Looking at these results, we can see that RHC has a very similar estimated effect here and in the univariate Cox PH model we fit above. In both cases, the effect of RHC is to increase the chance of death for the patient, with a hazards ratio of `r round(res_exp %>% filter(term == "RHC") %>% select(estimate), 2)`, whose confidence interval is `r round(res_exp %>% filter(term == "RHC") %>% select(conf.low), 2)` to `r round(res_exp %>% filter(term == "RHC") %>% select(conf.high), 2)`.
...
of hazard against time may indicate a violation of the assumption. 

The first plot we will consider is the "complementary 
log-log" model, which displays ($ln(-ln(1-h(t)))$ or $ln(-ln(S(t)))$ 
...
Looking at this plot, it appears that our dataset doesn't violate the assumption.


#### Statistical assessments 

Another approach to assess the model assumptions is to use a formal statistical test. Here we will 
use the `cox.zph()` function in the `survival` package. It tests
...
variable. If the interaction terms are non-significant, the conclusion is that the effect of that covariate does not vary with time. The function also performs a global test for all the 
interactions tested together in the model. If the p-value 
is less than 0.05, this suggests a violation of the PH
assumption. [see @Grambsch_1994]

For instance, if our model is 
$ln(h(t|X_1))= \beta_0 + \beta_1 X_1$ (where 
$\beta_0 = \ln(h_0(t))$), to test whether $X_1$ is 
time-varying, the `cox.zph()` function creates an 
interaction term $X_1*ln(t)$ in the model. 
Now the model is 

$$ln~h(t|X_1)= \beta_0 + \beta_1~X_1 + \beta_2(X_1*ln~t)$$

Using a null hypothesis of $H_0: \beta_2=0$, we can 
assess whether or not there is a relationship with time according to whether we reject or fail to reject this null hypothesis. Note, 
a p-value less than a specified Type 1 error (e.g. $\alpha$ 
...
use `transform = log` to get $X*ln~t$ in our model. 
...
We notice the p-value for `ph` is less than 0.05, 
...
`sex`,  `education`, `diseaseComa` and `diseaseCOPD`, as well as `ph`) and graphically assess 
whether or not there is a relationship between the variable and time. 
We will use the function `ggcoxzph` in the package `survminer`, 
...
The `grid.arrange()` function in the package `gridExtra` can be used to arrange several plots in an array. The `ggcoxzph` 
function, rather than generating plots, returns an object which is a list of ggplot objects, 
, or more specifically, gtables. 
These `gtables` are matrix-like layouts placed in cells that can
be directly used as an input in the `grid` system of graphics and related packages.
So to convert them to ggplot objects, we use the `ggplotGrob()` function in the package 
`ggplot2` to generate ggplot2 grobs that are compatible
...
interval around this smoothed line. As we said, an apparent departure from a horizontal 
line indicates violation for the proportional hazards assumption. All of these 6 
...
Therefore, even though we see a p-value less than 0.05 for 
...
variables, the p-values from some variables may be less 
than 0.05 by chance. Especially when there are lots of 
variables, a subtle deviation will be reflected in the p-value, whereas the graphical summary can give us a better feeling for the extent of the violation. 
...
df_res <- 
  data.frame(c1 = c("RHC", "RHC"),
        ...
        c3 = c(res_uni1$estimate, res_all %>% filter(term=="RHC") %>% select(estimate)),
        c4 = c(res_uni1$p.value, res_all %>% filter(term=="RHC") %>% select(p.value)),
...
We can then compare these three plots: 
...
for a matched dataset, corresponding to the univariate 
and multivariate models, respectively. In the multivariate 
...
Next we tried the Kaplan-Meier method, a non-parametric method, to explore 
...
Finally, we fit a mutlivariate Cox PH model to assess adjusted outcomes. In both the univariate and 
multivariate model, the difference of survival between the `RHC` and `No RHC` groups decreases with the multivariate model, 
...
for patients without RHC treatment, so we conclude that this treatment is harmful.
...
which increase the probability of receiving RHC and the probability of death overall make it difficult to accurately assess the impact of RHC based on anecdotal observation. Our analysis, which attempts to control for these confounders, aims to produce a more rigorous statistical result, within the limitations of this observational data set. 
