...
To show the survival curve, the function
...
list containing survival plot, the data used to plot plots, etc.
...
is `r round(p_km$data.survplot$surv[29],digits= 2)` 
and `r round(p_km$data.survplot$surv[58],digits= 2)` 
...
Now let's look at it in our data, we still use
`ggsurvplot()` to plot it. `fun` optionn is an
arbitrary function defining a transformation of 
the survival curve, we use `log(-log(x))` to 
...
res_all[31, ] # row contains result for RHC
```
...
by using a parametric method: Cox proportional hazards model. First
we applied univariate model and showed three examples. Then all necessary
variables were included, avoiding that we may ignore potential risks. 
We also evaluated the proportionality assumption both with graphical
checks and statistical test. There is no obvious pattern with time
so the assumption is supported with our data set. 

Furthermore, we adjusted outcomes. Whether with univariate or 
multivariate model, the difference of survival between two groups tenuates, 
proving the necessity of what we did (data matching); However, 
survival rate for patients with RHC treatment are still lower than
