...
survival (or mortality) due to  
treatment with a RHC, using an observational dataset. 
...
differences between the two groups below. 
...
at last follow-up (right-censored). Otherwise it is likely that the probability of death will be underestimated. 
[SHEENA: I think we need to clarify that the excusion is based on status at the time in question, right?]
...
SHEENA: I am getting a bunch of NAs in the output of survfit. Is that correct?
...
list containing the survival plot, the data used to create the plot, etc.
...
The information may include variables such as `age`, `gender` and `blood pressure` etc. To use propensity scores, we need a way to estimate these probabilities, and then figure out how to use these propensity scores to isolate the effect we are interested in measuring, i.e., the effect of RHC on patient survival. 
...
SHEENA: Is there a way to use names instead of c(1,2,5,17,18,30)?
...
SHEENA: Can you use a row name instead of 31?
