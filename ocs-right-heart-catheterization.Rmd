...
In general, the survival probability is calculated as 
...
$t_i$ is the time when at least one event happened. And 
this estimator, $\widehat{S(t)}$, also represents the 
probability that life is longer than $t$.
...
First we will fit a few univariate Cox PH models and 
then we will fit a multivariate Cox PH model using 
all the covariates in our dataset (`rhc_psm_df`). Both 
of these models are using the data after adjusting for
propensity scores.

#### Univariate Cox PH model

To apply univariate Cox PH models, we would just 
include one variable in the Cox PH model. 
We may choose `RHC`, `age` and `weight` as 
examples, and show fit results.

e.g. Univariate Cox PH model for `RHC`
...
e.g. Univariate Cox PH model for `age`
...
e.g. Univariate Cox PH model for `weight`
...
Compare the three results we find `RHC` has the strongest 
influence while `weight` has the weakest influence.


#### Multivariate Cox PH model

Next, we can fit the Cox PH model using the `coxph()` 
...
log-log" model ($log(-log(1-h(t)))$ or $ln(-ln(S(t)))$() 
...
rather than a plot `grob`, more specifically, gtables. 
...
interval. As we said, apparent departure from a horizontal 
line indicates violation for assumption. All of these 6 
variables have a zero slope, meaning the assumption is 
supported. We are likely to accept results provided by 
graphical dignostics and here is explanation [see @Klein_2006]:
...
p_all <- ggadjustedcurves(fit_all, data = rhc_psm_df,
      ...
      ylab = 'survival probability', 
      xlab = 'survival time', 
      title = 'Cox PH Plot-multivariate',
      ggtheme = theme_minimal())

grid.arrange(p_uni1, p_all, p_km$plot, ncol = 3)
```

Why we put `p_km$plot` in `grid.arrange()` function?
Similiar to the questions we met before, `p_km` is a list
created by funciton`ggsurvplot()` and it is not compatible with
`grid.arrange()`. Officially, a function 
called `arrange_ggsurvplots()` is used to arrange 
multiple ggsurvplots on the same page. 

But the other two plots are created by `ggadjustedcurves()` 
that are compatible with `grid.arrange()`. 
There is an easy way to solve it: extract `plot` object from
list `p_km`.


The left and middle plots show survival curves for matched dataset,
corresponding to univariate or multivariate model respectively.
In multivariate case, the difference of survival rate seems to
be smaller than that in univariate case, but it is subtle.

Compared with the right Kaplan-Meier plot, whether we use 
univariate or multivariate model, the difference of survival rate 
between these two groups tenuates; But patients in RHC group still
have lower survival rate than No RHC group so it is still harmful.
