...
Then we apply `survift()` to compute an estimate of a survival curve for censored data. Both of these two functions come from package [`survival`](https://www.rdocumentation.org/packages/survival/versions/2.11-4).
...
The total number of the patients having RCT is `r sum(rhc_df$RHC == 1, na.rm = TRUE)`, and `r sum(rhc_df$RHC == 1, na.rm = TRUE)-fit$n.censor[58]` of them died in 30 days(38%). The total number of the patients not having RCT is `r sum(rhc_df$RHC == 0, na.rm = TRUE)`, and `r sum(rhc_df$RHC == 0, na.rm = TRUE) - fit$n.censor[29]` of them died in 30 days(30.6%).
...
Now the distribution of propensity score are much more balanced! So the matching is done well. In addition to plot by yourself, there is another way to implement the result.
