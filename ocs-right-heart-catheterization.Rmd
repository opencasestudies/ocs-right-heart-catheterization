...
the column to a numeric. To do this we will use the `as.factor()` 
and `as.numeric()` functions to transform the factor as 
numerical variable. While we here, we will also change the data 
...
**Sheena** add text and code for univariate Cox PH models

e.g. 

```{r}
fit_rhc <- coxph(Surv(time, death) ~ RHC, 
              data = rhc_psm_df)

res <- tidy(fit_rhc, conf.int = TRUE,
                conf.level = 0.95)
res
```
...
**Sheena**: Add the code for the complementary log log plot here
...
we use the `fit1` Cox PH model from above. The `transform`
...
correlation between the scaled Schoenfeld residuals 
and `log(time)` for each covariate. The last row contains
...
rather than a plot grob. To convert them, we use `ggplotGrob()` 
function in package `ggplot2` to generate a ggplot2 plot 
grob that can be understood with function `grid.arrange()`.
...
**Sheena** using the survminer pacakge, add a survival plot 
with the curves split by `RHC`


If we compare that to the survival plot after adjusting for 
other covariates, we see that: 

**Sheena** using the survminer package, add a survival plot 
with the curves split by `RHC` after adjusting for covariates 
using cox PH model 
